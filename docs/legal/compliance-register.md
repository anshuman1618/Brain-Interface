...
operational rather than legal. Two are now resolved; **0.3 is the one still
costing you data.** They are here because a platform that loses a
...
| #   | Item                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Status |
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ |
| 0.1 | **RESOLVED 17 September 2026.** The free Postgres expired on 9 September and took the platform down for eight days: the instance suspended, its internal hostname stopped resolving, and `migrate-on-boot.mjs` refused to start the server — as designed. Recovered by upgrading the INSTANCE (not the workspace; they are billed separately) to `0.1c-256mb`, which restored it with its data intact and removed the expiry date entirely. `status: available`, no `expiresAt`. Standing lesson: a free database is a scheduled outage, and a workspace plan does not cover the instances inside it.                                                                                                                                                                                                      | Owner  |
| 0.2 | **RESOLVED 25 September 2026, and it had been resolved for eight days without anybody noticing.** The upgrade in 0.1 moved the database to a paid plan, and point-in-time recovery comes with every paid plan — a rolling **3-day** window at this tier. Nobody re-read the documents, so the Privacy Policy, DPA §4 and §7, the summary, the runbook and this row all went on telling chambers there were no backups from 17 to 25 September. All are corrected. **Standing lesson: an upgrade resolves the items it resolves silently.** When a plan changes, re-read §8's coupling list the same day. Two things still owed here: confirm the window on the dashboard's Recovery page, and **rehearse a restore** — an untested recovery capability is a claim, and the DPA now says so in those words. | Owner  |
| 0.3 | **Uploaded case files are written to the container filesystem.** `R2_*` is unset on the Render service — the Cloudflare bucket alone does nothing — so every document a chamber uploads is destroyed by the next deploy. The server warns at every boot. **Now also disclosed on 25 September 2026** in the Privacy Policy under "Retention", DPA §4 and the Data Usage Summary, rather than being a gap only we knew about. Disclosure is not a fix: these are privileged client files and the four variables are the fix.                                                                                                                                                                                                                                                                                | Owner  |
| 0.4 | **RESOLVED 24 September 2026.** The principal place of business is **Sector I, Jankipuram, Lucknow, Uttar Pradesh**, now stated in Terms §1 and §13 and in the Privacy Policy (operator identity and the postal contact row). It also fixed the governing-law seat: the courts at Lucknow have exclusive jurisdiction, which follows from §1 rather than having been chosen separately — worth confirming with counsel.                                                                                                                                                                                                                                                                                                                                                                                    | Owner  |
...
| §8(5)             | Reasonable security safeguards                            | See §4 below. Real, except for 0.3 — uploaded documents are still not on durable storage.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Partial |
...
| §3(c)(ii), §17(1) | Publicly available data, and judicial-function processing | **Cause lists.** A court's daily list is personal data about parties and counsel who are not our users. §3(c)(ii) puts data the individual has made publicly available outside the Act, and §17(1)(b) and (d) exclude processing for a judicial function and for enforcing a legal right; a court publishes its list under its own rules, and an advocate reading it to know when to appear is the purpose it exists for. Recorded in the Privacy Policy under "Court cause lists" so it can be argued with rather than assumed. **Sync is off** (`CAUSE_LIST_SYNC` unset), and nothing prunes fetched listings — both stated in the policy. | Partial |
...
| Address of the principal place of business                | Sector I, Jankipuram, Lucknow, Uttar Pradesh. Terms §1.                                                                                                                                                                                                                                                                                                                                                  | Done    |
...
| A published service level, so the offer is not overstated | **RESOLVED 25 September 2026.** Terms §8 now publishes a 99% monthly target, the maintenance window, the exclusions, and — the part that matters — that the deployment is one instance on a plan that sleeps when idle, and that uploaded documents are not on durable storage. An "as available" clause that does not say what is available is the unfair-practice risk, not the absence of a warranty. | Done    |
...
| Content-Security-Policy, opt-in per deployment              | `middlewares/securityHeaders.ts`. `CSP=off\|report-only\|enforce` + `CSP_CLERK_ORIGIN`; a live mode that cannot build a policy is refused at startup. **Not switched on yet.**                      |
| Point-in-time recovery, rolling 3 days                      | The paid database plan. **Restores unrehearsed** — see 0.2; an untested recovery capability is a claim, not a measure.                                                                              |
| A published route for reporting a flaw                      | `responsible-disclosure.md`, served at `/legal/disclosure`, with a safe harbour and stated triage times. Terms §5 points at it as the standing permission rather than forbidding research outright. |

**Not implemented, and claimed nowhere:** durable file storage (0.3), a
rehearsed restore (0.2), an enforced CSP, 180-day Indian log retention (§2), a
penetration test by a third party, and any certification. Do not let a sales
conversation imply otherwise.
...
behaviour the code does not have is a written admission.** These move together
or not at all:

- **The recovery window.** Privacy Policy "Retention", DPA §4 and §7, the Data
  Usage Summary in three places, Terms §8, the runbook's §1, item 0.2 here, and
  `DEPLOYMENT.md`. **Nine passages, which is the point:** they said "no backups"
  for eight days after the upgrade that gave us some, because a plan change
  resolves things silently. If the dashboard says the window is 7 days rather
  than 3, every one of these carries the number.
- **Document durability**, when `R2_*` is finally set. Privacy Policy
  "Retention", DPA §4, the Data Usage Summary in two places, Terms §8, item 0.3
  here, and the out-of-scope list in `responsible-disclosure.md`.
- **The CSP**, when it is switched on. §4's table here, `DEPLOYMENT.md` §6, and
  the disclosure policy's out-of-scope list, which currently tells researchers
  a missing CSP is known and not a finding.
- **The service level.** Terms §8 describes a single instance on a plan that
  sleeps. Moving the web service off the free plan makes two of its bullets
  wrong, and the Data Usage Summary's third point with them.
- **Cause lists.** Turning `CAUSE_LIST_SYNC` on makes "no court list is being
  fetched at the date of this policy" false. The §1 row and the Privacy Policy
  section change together, and the retention sentence — nothing prunes them —
  stops being acceptable the moment real lists start accumulating.
- **Logs to India** changes §2. **An email provider** changes two subprocessor
  tables and a note in each.
