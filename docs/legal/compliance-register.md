...
| A published service level, so the offer is not overstated | **RESOLVED 25 September 2026.** Terms §8 now publishes a 99% monthly target, the maintenance window, the exclusions, and — the part that matters — that the deployment is one instance on a plan that sleeps when idle, with no backups and no durable document storage. An "as available" clause that does not say what is available is the unfair-practice risk, not the absence of a warranty. | Done    |
...
- **Backups.** Privacy Policy "Retention", DPA §4 and §7, the Data Usage Summary
  ("The four things to know" and "How long we keep things"), item 0.2 here, and
  `DEPLOYMENT.md`.
- **Document durability**, when `R2_*` is finally set. Privacy Policy
  ...
  here, and the out-of-scope list in `responsible-disclosure.md` — which names
  both gaps as known, and must stop naming one on the day it is fixed.
