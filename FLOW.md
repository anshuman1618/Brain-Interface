...
| `practice-portal/src/components/court-listings.tsx` | The review queue: pending / accepted / dismissed, the listing as published, Accept and "Not ours". Plus the admin run log and manual "check now", both behind `audit.read`. **A component, not a page** — it renders inside the Master Calendar's second tab. |
...
| `practice-portal/src/pages/calendar.tsx`            | Hosts it, as two tabs — `Schedule` and `Court Listings`. Radix unmounts the inactive one, so the queue's queries do not run until the tab is opened.                                                                                                          |
| `practice-portal/.../layout/dashboard-layout.tsx`   | No nav entry any more. `/cause-list` is a `<Redirect to="/calendar">` so bookmarks survive.                                                                                                                                                                   |
