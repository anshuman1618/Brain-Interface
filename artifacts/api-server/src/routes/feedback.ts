 ...
 */
router.get("/feedback", requireWorkspace, requireCapability("feedback.read"), async (req: AuthRequest, res): Promise<void> => {
  const c = ctx(req);
  ...
  const rows = await db.select().from(feedbackTable).where(and(...conditions));
 ...
 */
router.post("/feedback", requireWorkspace, requireCapability("feedback.write"), async (req: AuthRequest, res): Promise<void> => {
  const c = ctx(req);
  ...
  const [existing] = await db
    .select()
    .from(feedbackTable)
    .where(and(eq(feedbackTable.caseId, matter.id), eq(feedbackTable.clientClerkId, c.user.clerkId)));
  if (existing) {
 ...
 */
router.post("/feedback/:id/response", requireWorkspace, requireCapability("feedback.respond"), async (req: AuthRequest, res): Promise<void> => {
  const c = ctx(req);
  ...
  const [updated] = await db
    ...
    .set({ response: parsed.data.response.trim(), respondedBy: c.user.displayName, respondedAt: new Date() })
