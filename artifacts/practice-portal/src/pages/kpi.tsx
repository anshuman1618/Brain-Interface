...
export default function KpiPage() {
  ...
  if (isLoading) {
    return <div className="space-y-4 p-8"><Skeleton className="h-10 w-48" /><Skeleton className="h-[400px] w-full" /></div>;
  }
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div>
        ...
        <p className="text-muted-foreground">Firm performance, SLA adherence, and pipeline metrics.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="rounded-none shadow-none border-border">
          ...
          <CardContent>
            <div className="text-4xl font-bold tracking-tighter">{kpi.avgTurnaroundDays} <span className="text-lg text-muted-foreground font-normal">days</span></div>
        ...
        <Card className="rounded-none shadow-none border-border">
          ...
          <CardContent className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={kpi.tasksByStatus || []} margin={{ top: 20, right: 30, left: 0, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="var(--border)" opacity={0.5} />
                <XAxis dataKey="status" tick={{fontSize: 12, fontFamily: 'monospace'}} axisLine={false} tickLine={false} tickFormatter={(v) => v.toUpperCase().replace('_', ' ')} />
                <YAxis tick={{fontSize: 12, fontFamily: 'monospace'}} axisLine={false} tickLine={false} />
                <RechartsTooltip cursor={{fill: 'var(--muted)'}} contentStyle={{borderRadius: 0, border: '1px solid var(--border)', backgroundColor: 'var(--background)'}} />
          ...
          <CardContent className="h-[300px] flex items-center justify-center">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                <RechartsTooltip contentStyle={{borderRadius: 0, border: '1px solid var(--border)', backgroundColor: 'var(--background)'}} />
              </PieChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
