...
import {
  ...
  FileText, Upload, ListTodo, Archive, Check, Plus, Send, UserRound
} from "lucide-react";
    ...
    <div className="space-y-8 animate-in fade-in zoom-in-95 duration-500">
      ...
      <div className="pt-4 border-t border-border">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {can("tasks.write") && (
            <button onClick={() => setTaskFormOpen(true)} className="bg-slate-900 border-l-4 border-slate-400 text-white rounded-none p-4 flex flex-col items-center justify-center gap-3 hover:opacity-90 transition-opacity animate-in fade-in slide-in-from-bottom-4 duration-500">
              <Plus className="h-8 w-8" />
              <span className="font-mono uppercase text-xs font-bold tracking-wider text-center">Create / Assign Task</span>
            </button>
          )}
      ...
      {can("document_requests.create") && (
        <div className="pt-4 border-t border-border">
          <div className="flex items-center justify-between mb-4 gap-4">
            ...
            <Button variant="outline" className="rounded-none shrink-0" onClick={() => setDocRequestOpen(true)}>
          ...
          {outstandingRequests.length === 0 ? (
            ...
            <div className="space-y-2">
              {outstandingRequests.slice(0, 5).map((req) => (
                <div key={req.id} className="p-4 border border-border bg-background flex flex-col sm:flex-row sm:items-center gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground mt-1 font-mono uppercase tracking-wider flex flex-wrap items-center gap-x-2 gap-y-1">
                      ...
                      <span>&middot;</span>
                      <span>Asked by {req.requestedBy}{req.requestedByRole ? ` (${req.requestedByRole})` : ""}</span>
                      {req.dueDate && (
                        <>
                          <span>&middot;</span>
                          <span className={new Date(req.dueDate) < new Date() ? "text-destructive font-bold" : ""}>
                    ...
                    {req.caseTitle && (
                      <p className="text-xs text-muted-foreground mt-1 truncate">Matter: {req.caseTitle}</p>
                    )}
                  </div>
              ...
              <div key={req.id} className="bg-slate-700/50 p-4 border border-slate-600 flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
                <div className="min-w-0">
                  ...
                  <div className="text-sm text-slate-300 font-mono mt-1 space-y-1">
                    ...
                    {req.dueDate && (
                      <p className={new Date(req.dueDate) < new Date() ? 'text-red-300 font-bold' : ''}>
                        Needed by: {new Date(req.dueDate).toLocaleDateString()}
                      </p>
                    )}
                    {req.note && <p className="italic opacity-80">"{req.note}"</p>}
                    <p className="text-xs opacity-60 uppercase tracking-widest">Raised {new Date(req.createdAt).toLocaleDateString()}</p>
                  </div>
                </div>
