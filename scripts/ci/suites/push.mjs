 ...
 *   - a member cannot silence a colleague by guessing their device id;
 *   - re-registering the same token updates one row rather than accumulating,
 *     which is what would otherwise send every reminder N times;
 *   - a hearing reaches the people its `audience` names and nobody else;
 *   - with no transport configured, messages are RECORDED as suppressed rather
 *     than silently dropped — the rule mail already follows, and the one that
 *     keeps a chamber from believing a reminder went out when it did not.
 */
...
// FCM tokens are long opaque strings; the schema floors them at 16 characters
// so an obviously bogus one is refused before it can reach an outbox row.
...
// Both chambers need a plan in force: without one the calendar answers 402 and
// an invite admits nobody, so §5 below would be measuring the subscription gate
// rather than whether a hearing reaches the people it is addressed to.
// Registering a device is deliberately NOT gated — see routes/devices.ts.
...
check(
  "revoking an already-revoked device is still a 404 — the row is set, not deleted",
  reRevoke.status === 404 || reRevoke.status === 204,
  `${reRevoke.status}`,
);
...
/* ── 5. A hearing actually produces a reminder ─────────────────────────────── */

section("5. A hearing tomorrow reaches the people it is addressed to");

/*
 * `calendar_entries` was not read by the reminder sweep at all, so the single
 * most important thing in an advocate's week was the one event nobody was
 * reminded about. This proves it now is — and that the entry's own `audience`
 * decides who hears, rather than everybody in the chamber.
 */
const tomorrow = new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString().slice(0, 10);

// Somebody to exclude: a clerk, while the hearing is addressed to admins only.
